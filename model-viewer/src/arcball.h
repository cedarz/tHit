...
// https://github.com/iauns/cpm-arc-ball
...
public:
  ...
  ArcBall(const glm::vec3& center, float radius,
          const glm::mat4& screenToTCS = glm::mat4(1.0));
  virtual ~ArcBall();
  ...
  glm::vec3 map_to_sphere(const glm::vec2& point2D)
  {
      double x = point2D.x;
      double y = 1.0 - point2D.y;
          //double sinx = sin(std::numbers::pi * x * 0.5);
          //double siny = sin(std::numbers::pi * y * 0.5);
          //double sinx2siny2 = sinx * sinx + siny * siny;
          glm::vec3 result(x, y, 0.0);
          //result[0] = sinx;
          //result[1] = siny;
          //result[2] = sinx2siny2 < 1.0 ? sqrt(1.0 - sinx2siny2) : 0.0;

          double sum = x * x + y * y;
          result.z = sum <= 0.5 ? sqrt(1.0 - sum) : (0.5 / sqrt(sum));
          return result;
  }
