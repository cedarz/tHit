 ...
 *
 * Adapted to use glm types instead of GLSL-PathTracer types
 */
...
bool LoadScene(const std::string& filename, Scene* scene, RenderOptions& renderOptions)
{
    FILE* file;
    ...
    while (fgets(line, kMaxLineLength, file)) {
        ...
        if (sscanf(line, " material %s", name) == 1) {
            ...
            // Albedo Texture
            if (strcmp(albedoTexName, "none") != 0) material.baseColorTexId = scene->AddTexture(path + albedoTexName);

            // MetallicRoughness Texture
            if (strcmp(metallicRoughnessTexName, "none") != 0) material.metallicRoughnessTexID = scene->AddTexture(path + metallicRoughnessTexName);

            // Normal Map Texture
            if (strcmp(normalTexName, "none") != 0) material.normalmapTexID = scene->AddTexture(path + normalTexName);

            // Emission Map Texture
            if (strcmp(emissionTexName, "none") != 0) material.emissionmapTexID = scene->AddTexture(path + emissionTexName);
        ...
        if (strstr(line, "camera")) {
            ...
            while (fgets(line, kMaxLineLength, file)) {
                ...
                // Read matrix (column-major order for glm)
                float m[16];
                if (sscanf(line, " matrix %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f", &m[0], &m[4], &m[8], &m[12], &m[1], &m[5], &m[9], &m[13], &m[2], &m[6], &m[10], &m[14], &m[3], &m[7], &m[11], &m[15]) == 16) {
                    xform = glm::mat4(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9], m[10], m[11], m[12], m[13], m[14], m[15]);
                    matrixProvided = true;
                }
            }

            if (matrixProvided) {
                glm::vec3 forward = glm::vec3(xform[2][0], xform[2][1], xform[2][2]);
                position = glm::vec3(xform[3][0], xform[3][1], xform[3][2]);
        ...
        if (strstr(line, "renderer")) {
            ...
            if (!renderOptions.independentRenderSize) renderOptions.windowResolution = renderOptions.renderResolution;
        }
        ...
        if (strstr(line, "mesh")) {
            std::string filename;
            glm::quat rotQuat;
            glm::mat4 xform(1.0f), translate(1.0f), rot(1.0f), scale(1.0f);
            ...
            while (fgets(line, kMaxLineLength, file)) {
                ...
                glm::vec3 pos;
                if (sscanf(line, " position %f %f %f", &pos.x, &pos.y, &pos.z) == 3) translate = glm::translate(glm::mat4(1.0f), pos);

                glm::vec3 scl;
                if (sscanf(line, " scale %f %f %f", &scl.x, &scl.y, &scl.z) == 3) scale = glm::scale(glm::mat4(1.0f), scl);

                if (sscanf(line, " rotation %f %f %f %f", &rotQuat.x, &rotQuat.y, &rotQuat.z, &rotQuat.w) == 4) rot = glm::mat4_cast(rotQuat);
            }
        ...
        //--------------------------------------------
        // Model, multiple material and multiple Mesh

        if (strstr(line, "model")) {
            std::string filename;
            glm::quat rotQuat;
            glm::mat4 xform(1.0f), translate(1.0f), rot(1.0f), scale(1.0f);
            int material_id = -1; // 0 for Default Material ID
            char meshName[200] = "none";
            bool matrixProvided = false;
            ...
            if (!filename.empty()) {
                ...
                //MeshInstance instance(instanceName, mesh_id, transformMat, material_id);
                scene->AddModel(filename, transformMat, material_id);
            }
        }

        //--------------------------------------------
        // GLTF - Note: GLTF support is not included in this port
        // To add GLTF support, you would need to integrate GLTFLoader separately
    }
