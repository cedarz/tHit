...
#pragma omp parallel for
	for (int i = 0; i < ns; i++) {
		...
		if (!n) {
			...
			tangents[i].push_back(glm::vec3(0,0,-1));
			continue;
		}
		tangents[i].resize(n);
		for (int j = 0; j < n-1; j++) {
            glm::vec3 t = strands[i][j + 1] - strands[i][j];
			t = glm::normalize(t);
			tangents[i][j] += t;
			tangents[i][j+1] += t;
		}
		for (int j = 0; j < n; j++)
			tangents[i][j] = glm::normalize(tangents[i][j]);
#if 0
...
void Hair::subdiv()
{
	...
	for (int i = 0; i < nstrands; i++) {
		...
		for (int j = 0; j < nverts - 1; j++) {
            ...
            glm::vec3 p = 0.5f * (oldstrand[j] + oldstrand[j + 1]);
			if (j > 0)
