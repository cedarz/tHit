...
void TrackballViewer::display()
{
    ...
    glm::vec4 mc(center_, 1.0);
    glm::vec4 ec = modelview_matrix_ * mc;
    float z = -ec[2];
    near_ = 0.01 * radius_;
    far_ = 10.0 * radius_;
    fovy_ = 45.0;
    ...
    projection_matrix_ = glm::perspective(fovy_, (float)width() / (float)height(), near_, far_);
...
void TrackballViewer::init()
{
    ...
    // init modelview
    modelview_matrix_ = glm::identity<glm::mat4>();
...
void TrackballViewer::set_scene(const glm::vec3& center, float radius)
{
    center_ = center;
    radius_ = radius;
    view_all();
...
void TrackballViewer::view_all()
{
    ...
    glm::vec4 t = modelview_matrix_ * c;
    translate(glm::vec3(-t[0], -t[1], -t[2] - 2.5 * radius_));
}

bool TrackballViewer::pick(glm::vec3& result)
{
    double x, y;
    cursor_pos(x, y);
...
bool TrackballViewer::pick(int x, int y, glm::vec3& result)
{
    ...
    if (zf != 1.0f)
    {
        ...
        glm::mat4 mvp = projection_matrix_ * modelview_matrix_;
        glm::mat4 inv = inverse(mvp);
        glm::vec4 p = inv * glm::vec4(xf, yf, zf, 1.0f);
        p /= p[3];

        result = glm::vec3(p[0], p[1], p[2]);
...
bool TrackballViewer::map_to_sphere(const glm::ivec2& point2D, glm::vec3& result)
{
...
void TrackballViewer::translation(int x, int y)
{
    ...
    glm::vec4 mc = glm::vec4(center_, 1.0);
    glm::vec4 ec = modelview_matrix_ * mc;
    float z = -(ec[2] / ec[3]);
...
void TrackballViewer::translate(const glm::vec3& t)
{
    glm::mat4 m;
    m = glm::translate(m, t);
    modelview_matrix_ = m * modelview_matrix_;
}

void TrackballViewer::rotate(const glm::vec3& axis, float angle)
{
    ...
    glm::mat4 IT;
    IT = glm::translate(IT, c);
    modelview_matrix_ = T * R * IT * modelview_matrix_;
}
