...
public:
    ...
    using apiHandle = GLuint;
    static const uint32_t kMaxPossible = -1;

    static SharedPtr create2D(uint32_t width, uint32_t height, ResourceFormat format,
        uint32_t arraySize = 1, uint32_t mipLevels = kMaxPossible, const void* pInitData = nullptr,
        ResourceBindFlags bindFlags = ResourceBindFlags::ShaderResource);
    
    Texture(uint32_t width, uint32_t height, uint32_t depth, uint32_t arraySize,
        uint32_t mipLevels, uint32_t sampleCount, ResourceFormat format, TexType Type, ResourceBindFlags bindFlags);

    apiHandle getApiHandle() const { return m_api_handle; }
...
public:
     ...
     * @param[in] depth A flag to indicate to generate a depth buffer.
    ...
    operator bool() const noexcept { return m_api_handle != INVALID; }
    int width() const noexcept { return m_width; }
    int height() const noexcept { return m_height; }
    int samples() const noexcept { return m_sample_count; }
    ...
    gl::Handle native_handle() const noexcept { return m_api_handle; }
